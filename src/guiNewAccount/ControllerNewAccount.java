	...
	*/
	...
	protected static void doCreateUser() {
		...
		if (ViewNewAccount.text_Password1.getText().
			...
			String evaluated_uname = UserNameRecognizer.checkForValidUserName(username);
			if (!evaluated_uname.isEmpty()) {

				ViewNewAccount.usernameInvalidAlert.setTitle("Invalid Username");
				ViewNewAccount.usernameInvalidAlert.setHeaderText("Username does not meet requirements.");
				ViewNewAccount.usernameInvalidAlert.setContentText("Correct the username and try again.\n" + evaluated_uname);
				ViewNewAccount.usernameInvalidAlert.showAndWait();
				return;
	        }
			
			if (theDatabase.getUserAccountDetails(username)) {

				ViewNewAccount.usernameTakenAlert.setTitle("Username already taken");
				ViewNewAccount.usernameTakenAlert.setHeaderText("Username does not meet requirements.");
				ViewNewAccount.usernameTakenAlert.setContentText("That username is already taken. Please choose another.\n" + evaluated_uname);
				ViewNewAccount.usernameTakenAlert.showAndWait();
			    return;
			}
			
			String evaluated_pwd = Model.evaluatePassword(password);
			if (!evaluated_pwd.isEmpty()) {

				ViewNewAccount.passwordInvalidAlert.setTitle("Invalid Password");
				ViewNewAccount.passwordInvalidAlert.setHeaderText("Password does not meet requirements.");
				ViewNewAccount.passwordInvalidAlert.setContentText("Correct the password and try again.\n" + evaluated_pwd);
				ViewNewAccount.passwordInvalidAlert.showAndWait();
				return;
	        }
            ...
            try {
            	...
            	applicationMain.FoundationsMain.isFirstAdminSetup = true; //Invited users go to login
