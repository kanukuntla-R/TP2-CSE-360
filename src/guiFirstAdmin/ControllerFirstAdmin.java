	...
	*/
	...
	protected static void doSetupAdmin(Stage ps, int r) {
		...
		if (adminPassword1.compareTo(adminPassword2) == 0) {
			...
			String evaluated_uname = UserNameRecognizer.checkForValidUserName(adminUsername);
			if (!evaluated_uname.isEmpty()) {
				ViewFirstAdmin.label_PasswordsDoNotMatch.setText("Invalid username: " + evaluated_uname);
				return;
			}
			
			
			String evaluated_pwd = Model.evaluatePassword(adminPassword1);
			if (!evaluated_pwd.isEmpty()) {
	            ViewFirstAdmin.label_PasswordsDoNotMatch.setText("Invalid password: " + evaluated_pwd);
				return;
	        }
