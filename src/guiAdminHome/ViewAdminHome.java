 ...
 *
 * <p> Description: Admin Home Page (JavaFX).</p>
 *
 * <p> Copyright: Lynn Robert Carter © 2025 </p>
 *
 * @author Lynn Robert Carter
 *
 * @version 1.02  2025-09-25  Use ChoiceDialog for OTP user selection inside the dialog.
 * @version 1.01  2025-09-25  (previous) OTP console logging and refresh logic.
 ...
 * 
 * @version 1.00		2025-08-17 Initial version
 * @version 1.02		2025-09-25  updated otp- password 
 *  
 */
...
public class ViewAdminHome {
	...
	*/
	...
	private ViewAdminHome() {
		...
		button_ManageInvitations.setOnAction((event) -> { ControllerAdminHome.manageInvitations(); });

		setupButtonUI(button_SetOnetimePassword, "Dialog", 16, 250, Pos.CENTER, 20, 320);
		button_SetOnetimePassword.setOnAction(e -> {
			// Build list of users for dropdown (remove placeholder if present)
			List<String> users = new ArrayList<>(theDatabase.getUserList());
			if (!users.isEmpty() && users.get(0).startsWith("<")) {
				users.remove(0);
			}
			if (users.isEmpty()) {
				...
				a.setContentText("There are no users to select.");
				a.showAndWait();
				return;
			}

			//This method is for the drop down in to generate otp password
			ChoiceDialog<String> dialog = new ChoiceDialog<>(users.get(0), users);
			dialog.setTitle("Set One-Time Password");
			dialog.setHeaderText("Select the user to generate a temporary one-time password for:");
			dialog.setContentText("User:");

			dialog.showAndWait().ifPresent(u -> {
				String otp = applicationMain.FoundationsMain.database.generateOneTimePasswordFor(u);

				// Console logging, similar to invitation flow
				if (otp != null) {
					System.out.println("** Admin generated OTP for username: " + u + " | OTP: " + otp);
				} else {
					System.out.println("** Admin attempted OTP for unknown username: " + u);
				}
				...
				if (otp == null) {
					...
					a.setHeaderText(null);
					a.setContentText("No such user: " + u);
				} else {
					a.setTitle("OTP Generated");
					a.setHeaderText("Share this OTP securely with the user. It’s valid for exactly one successful login.");
					a.setContentText("Username: " + u + "\nOne-Time Password: " + otp);
				}
				a.showAndWait();
			});
		});
